...
void AAGameSlot::SetState(EGridState NewState)
{
	GridState=NewState;
...
void AAGameSlot::SpawnUnitHere(TSubclassOf<AUnitBase>& UnitClass)
{
	FVector Location =GetActorLocation();
	AUnitBase*NewUnit =Cast<AUnitBase>(
		GWorld->SpawnActor(UnitClass, &Location));
...
{
	Super::BeginPlay();
	OnClicked.AddUniqueDynamic(this,&AAGameSlot::OnGridClicked);
	
}

void AAGameSlot::OnGridClicked(AActor* TouchedActor, FKey ButtonPressed)
{
